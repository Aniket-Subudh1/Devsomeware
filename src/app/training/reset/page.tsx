...
function SignupFormDemo() {
  ...
  //extra security
 useEffect(() => {
 if(!localStorage.getItem("studentAttendanceToken")){
    toast.error("No active session found please login to continue");
    setTimeout(() => {
      router.push("/attendance");
    }, 1000);
 }
 else{
  let token =  localStorage.getItem("studentAttendanceToken");
    ...
    if(data.status===false){
      ...
      if (typeof data.data === "object" && "email" in data.data) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const fetchdata = await fetch("/api/attendance/student/reset", {
      method: "POST",
      headers: {
...
<p className="text-neutral-600 text-sm max-w-sm mt-2 dark:text-neutral-300">
  Enter your current password , new password and confirm it below to reset your password. Make sure to choose a strong and secure password! 🔒
</p>
<p className="text-neutral-600 text-sm max-w-sm mt-2 dark:text-neutral-300">
  Password Change for : {localStorage.getItem("studentAttendanceEmail")}
</p>
