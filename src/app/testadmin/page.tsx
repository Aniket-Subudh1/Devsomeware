...
const mockUpdateStatus = async (id: string, newStatus: string) => {
  try{
...
refreshData();
  ...
  const handleStatusChange = async () => {
    ...
    try {
     ...
     const data =  await mockUpdateStatus(currentItem._id, newStatus);
      if (!data?.success) {
        ...
        refreshData();
        setSuccessMessage(`Status updated successfully for Round ${currentItem.round}`);
        // Close modal
        setShowModal(false);
      }
    } catch (err) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto bg-gray-900 min-h-screen mt-10 rounded">
      ...
      <div className="mb-6 flex justify-between items-center">
        ...
        <button 
          onClick={refreshData}
          className="flex items-center gap-2 px-4 py-2 bg-black border border-gray-300 rounded-lg shadow-sm hover:bg-gray-800 transition-colors"
        >
                ...
                >
                  <span 
                    className={`inline-block h-5 w-5 transform rounded-full bg-white shadow-md transition-transform ${
                      item.status === "true" ? "translate-x-6" : "translate-x-1"
                    }`} 
                  />
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fade-in">
          ...
          >
            ...
            <p className="mb-5 text-gray-600">
              You are about to change the status of <span className="font-bold">Round {currentItem?.round}</span> from{" "}
              <span className={`font-semibold ${currentItem?.status === "true" ? "text-green-600" : "text-gray-500"}`}>
                {currentItem?.status === "true" ? "Active" : "Inactive"}
              </span>
              {" "}to{" "}
              <span className={`font-semibold ${currentItem?.status === "true" ? "text-gray-500" : "text-green-600"}`}>
