...
export default function AttendanceAdminDashboard() {
  ...
  `/api/attendance/admin/records?password=${encodeURIComponent(password || "")}`
);
...
return (
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      // Define interfaces for Excel data
        ...
        filteredRecords.forEach((record) => {
          allData.push({
            ...
            "Check Out": record.checkOutTime
              ? new Date(record.checkOutTime).toLocaleTimeString()
              : "N/A",
            "Duration (mins)": record.duration ? record.duration.toString() : "0",
      ...
      if (campusFilter === "all") {
        campuses.forEach((campus) => {
          const campusData: CampusExcelData[] = [];
            ...
            .forEach((record) => {
              campusData.push({
                ...
                "Check Out": record.checkOutTime
                  ? new Date(record.checkOutTime).toLocaleTimeString()
                  : "N/A",
                "Duration (mins)": record.duration || "0",
                "Status": record.status
              });
            });
